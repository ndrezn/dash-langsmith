...
def _incoming_headers() -> dict:
    """Return the current Flask request headers, or {} outside a request context."""
    try:
...
def mcp_traced(
    ...
    *,
    ...
    Args:
        ...
        environment: Environment tag (e.g. "production", "staging") recorded in run
            metadata. Falls back to the LANGSMITH_ENVIRONMENT env var when unset.
    """

    def decorator(fn):
        _traced = langsmith.traceable(
            name=name or fn.__name__,
            run_type="tool",
            project_name=project_name,
        )(fn)

        env = environment or os.environ.get("LANGSMITH_ENVIRONMENT")

        @functools.wraps(fn)
        def wrapper(*args, **kwargs):
            headers = _incoming_headers()
            extra: dict = {"parent": headers}
            if env:
                extra["metadata"] = {"environment": env}
            return _traced(*args, langsmith_extra=extra, **kwargs)
        ...
        if mcp_kwargs:
            return _dash_mcp_enabled(**mcp_kwargs)(wrapper)
        return _dash_mcp_enabled(wrapper)
